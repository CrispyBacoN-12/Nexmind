// Blind test for research-68 (RSI Extreme Fade, low-ADX range, ADX<25 gate,
// gold): 57 trades, PF 1.29, +$15.46 on the 2y in-sample backtest - the
// survivor after widening data killed v1's edge (research-65/66). Same
// held-out methodology: test on GC=F 1h bars OLDER than the most recent
// 365 days.
// Usage: npx tsx scripts/blind-test-rsi-range-fade.mts
...
async function main() {
  for (const range of ["2y", "5y"] as const) {
    ...
    console.log(
      `RSI Extreme Fade (ADX<25)  trades=${String(result.trades.length).padStart(4)} win%=${winRate.toFixed(1).padStart(5)} ` +
