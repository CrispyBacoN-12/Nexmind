// Blind test for research-82 (Bollinger Band Reclaim Fade, gold): 132 trades,
// PF 1.03, win 56.1% on the 1y in-sample backtest - marginal but the least-
// bad range candidate found using the new BB/Stochastic indicators. Same
// held-out methodology as every other blind test this session: GC=F 1h bars
// OLDER than the most recent 365 days.
// Usage: npx tsx scripts/blind-test-bb-reclaim.mts
...
if (p.bbPercentB > 1 && s.bbPercentB <= 1) return { side: "short", note: "reclaimed inside upper band" };
if (p.bbPercentB < 0 && s.bbPercentB >= 0) return { side: "long", note: "reclaimed inside lower band" };
return null;
`;
...
async function main() {
  for (const range of ["2y", "5y"] as const) {
    ...
    console.log(
      `Bollinger Band Reclaim Fade  trades=${String(result.trades.length).padStart(4)} win%=${winRate.toFixed(1).padStart(5)} ` +
