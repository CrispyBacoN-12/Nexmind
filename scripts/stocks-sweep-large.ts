// Re-sweeps the same 8-candidate library used in stocks-sweep-candidates.ts,
// but against a ~50-symbol sector-diverse sp500 sample (every 10th symbol in
// the SP500 list) instead of the original 10 mega-cap names. The original
// 10-stock sample turned out to be unrepresentative - the winning pick
// ("ADX-Ignition Breakout") looked STABLE+ there but fails split-half
// robustness on this wider sample (stocks-large-sample-check.ts). Finding
// whatever actually holds up broadly, not just on a lucky small sample.
// Usage: npx tsx scripts/stocks-sweep-large.ts
...
if (s.plusDI > s.minusDI && s.price > s.sma20 && s.sma20 > s.sma50 && s.macdHist > p.macdHist && s.macdHist > 0) return { side: "long", note: "strong trend rider" };
if (s.minusDI > s.plusDI && s.price < s.sma20 && s.sma20 < s.sma50 && s.macdHist < p.macdHist && s.macdHist < 0) return { side: "short", note: "strong trend rider" };
return null;
`,
  },
  {
...
async function main() {
  console.log(`Sweeping ${CANDIDATES.length} candidates across ${SAMPLE.length} sector-diverse sp500 symbols (${INTERVAL}/${RANGE}, pooled)\n`);
